...
class JqgridRequest
  ...
  def self.add(jqgrid_id,colmodel)
    ...
    if pcolmodel.length>0
      pcolmodel.each do |x|
# JR -- why? this is definitely 1/2 baked
#        puts "ELFN:" + elfed    + ":"
#        puts "ELFS:" + x["elf"] + ":"
        if x["elf"]!=elfed
          x["elf"]      = elfed
...
# conflicts with app. application controller
class GridApplicationController < ActionController::Base
...
#        logger.info("FILTERING!!!")
        flt = mysql_escape(params["filter"])
      end
      sch = ""
...
#        logger.info("SEARCHING!!!")
      ...
      where = ""
      if flt.size>0 or sch.size>0
        where = ",:conditions => '"
        if flt.size>0 and sch.size>0
          where += flt + " AND " + sch
        else
          where += flt + sch
        end
      ...
      cex = model + '.count(:all' + where + ')'
      logger.info(cex)
      rcount = eval(cex)

      ex = model + '.find(:all' + where
      irows = rows.to_i
      unless (irows<1)
        ex = ex + ',:limit => ' + rows 
        ...
        if (rid==rid.gsub(/[^\d]/,''))
          ...
          if HVModels.isModel(model)
            ...
            if changed
              ...
              render :text => '', :status => 304    # not sure this is right, but better than 400 for a request that is NOT bad
            end
          else
            render :text => '', :status => 406
          end
